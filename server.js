...
    // Proxy settings need to be configured at the environment level for this library
...
    if (error.message && (error.message.includes('disabled') || error.message.includes('No transcript found') || error.message.includes('blocked'))) {
        // Added 'blocked' as a potential indicator if the error message changes
        return res.status(404).json({ error: `Failed to fetch transcript: ${error.message}. This may be due to IP blocking or transcript availability.` });
    }
